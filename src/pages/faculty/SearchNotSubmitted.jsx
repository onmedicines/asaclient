...
export default function SearchSubmitted() {
  ...
  async function handleSubmit(e) {
    try {
      ...
      const DYNAMIC_URL = `http://localhost:3000/faculty/getAllNotSubmitted?code=${selectedCode}`;
      setIsLoading(true);
  ...
  async function handleView(e) {
    try {
      ...
      const token = localStorage.getItem("token");
      let Dynamic_URL = `http://localhost:3000/faculty/getAssignment?code=${encodeURIComponent(selectedCode)}&rollNumber=${rollNumber}`;
      setIsLoading(true);
      const response = await fetch(Dynamic_URL, {
        method: "get",
        headers: {
